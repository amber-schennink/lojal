...
<head>
  ...
  <title>Lojal - Nieuws Aanpassen</title>
...
<body>
  ...
  <div class="content-wrapper">
    ...
    </header><script>var fileName =  window.location.pathname.split('/').pop();</script>
    <form id="form" onsubmit="return submitForm(this)" action="../nieuws-beheer/edit-nieuws-artikel.php" method="post" enctype="multipart/form-data">
      <input id="invisibleInputNameFile" name="nameFile" class="display-none" value="" />
      <script>document.getElementById('invisibleInputNameFile').value = window.location.pathname.split('/').pop();</script>
      <section class="wrapper bg-soft-primary">
        <div class="container pt-10 pb-19 pt-md-14 pb-md-20 text-center">
          <div class="row">
            <div class="col-md-10 col-xl-8 mx-auto">
              <div class="post-header">
                <input class="form-control primary display-1 mb-4 w-fit mx-auto" style="max-width: 90%;" type="text" name="titel" placeholder="Titel*" value="<?php echo $title ?>" required>
                <ul class="post-meta mb-5">
                  <li class="post-date">
                    <input class="form-control primary-light" type="text" name="schrijver" placeholder="Schrijver*" value="<?php echo $schrijver ?>" required>
                  </li>
                  <li class="post-date">
                    <input class="form-control primary-light" type="date" name="datum" value="<?php echo date("Y-m-d", strtotime($datum)) ?>" required>
                  </li>
                </ul>
              </div>
            </div>
              ...
              <div class="blog single mt-n17">
                <div class="card">
                  <div class="position-absolute right-0 flex gap-4" style="top: -40px;">
                    <p>Foto</p>
                    <label class="switch">
                      <input id="checkboxHeaderType" name="headerType" onClick="switchFotoVideo(this)" <?php if ($headerFotoType == 'video') {echo "checked='checked'";} ?> value="Video" type="checkbox">
  ...
  <div id="addPopUp" onclick="this.classList.add('display-none')" class="display-none position-fixed w-screen h-screen bg-half-gray top-0 left-0 z-full">
    <div onclick="event.stopPropagation()" class="bg-soft-primary w-fit h-90 m-auto position-absolute tblr-0 border-radius-04 p-5">
      ...
      <div class="display-grid grid-auto-auto column-gap-10 row-gap-2">
        <button class="addButton btn btn-primary-light" onclick="addTextBlockInput(2); document.getElementById('addPopUp').classList.add('display-none')">Nieuw tekst blok</button>
        <img class="addImgs" onclick="addTextBlockInput(2); document.getElementById('addPopUp').classList.add('display-none')" src="../assets/imgs/nieuwsTekst.png">
        <button class="addButton btn btn-primary-light" onclick="addImagesInput(4); document.getElementById('addPopUp').classList.add('display-none')">Nieuw fotos blok</button>
        <img class="addImgs" onclick="addImagesInput(4); document.getElementById('addPopUp').classList.add('display-none')" src="../assets/imgs/nieuwsFotos.png">
  ...
  <script type="text/javascript">
    ...
    <?php 
      if ($headerFotoType == 'video') {
        echo "switchFotoVideo(document.getElementById('checkboxHeaderType'));";
        echo"
        ...
        video.pause();
        blahVideo.src = 'uploads-video/" .$headerVideo. "';
        video.load();

        videoLabel.style.display = 'contents';
        
        var videoList = new DataTransfer();
        var videoFile = new File(['content'], 'uploadVideo/".$headerVideo. "');
        videoList.items.add(videoFile);
        var myvideoFileList = videoList.files;
        inputVideo.files = myvideoFileList;";

        if(isset($headerVideoPoster)){
          echo "blah.src = 'uploads/" .$headerVideoPoster. "';
          svgFoto.classList.add('display-none');
          blah.classList.remove('display-none'); 
          
          video.pause();
          video.poster = 'uploads/" .$headerVideoPoster. "';
          video.load(); 
          
          var fotoList = new DataTransfer();
          var fotoFile = new File(['content'], 'uploads/".$headerVideoPoster. "');
          fotoList.items.add(fotoFile);
          var myFotoFileList = fotoList.files;
          inputFoto.files = myFotoFileList;";
        }
      }else if(isset($headerFoto) && $headerFoto != ''){
        echo "blah.src = 'uploads/" .$headerFoto. "';
          svgFoto.classList.add('display-none');
          blah.classList.remove('display-none'); 
          ...
          var fotoList = new DataTransfer();
          var fotoFile = new File(['content'], 'uploads/".$headerFoto. "');
          fotoList.items.add(fotoFile);
          var myFotoFileList = fotoList.files;
          inputFoto.files = myFotoFileList;";
      }
    ...
    var numberImg = 0
    function addImagesInput(numberOfFotos) {
    ...
    function addSingleImgInput() {
      ...
      blog.appendChild(newGroup);
      return numberSingleImg;
    }
    var numberText = 0
    function addTextBlockInput(numberOfTextInputs) {
      var blog = document.getElementById('blog');
      ...
      input.setAttribute('id', 'textInputKop-' + numberText);
      ...
      for (let i = 0; i < numberOfTextInputs; i++) {
    ...
    <?php
      ...
      foreach ($blog as $value) {
        if($value['Type'] == 'Text'){
          echo "var id = addTextBlockInput(" .count($value['Text']). ");
          document.getElementById('textInputKop-' + id).value = '" .$value['Title']. "';
          var i = 1;";
          foreach ($value['Text'] as $text) {
            echo "document.getElementById('inputTextarea-' + id + '-' + i ).getElementsByTagName('textarea')[0].value = `" . $text . "`;
            i++;";
          }
          echo "setTextareas();";
        }else if($value['Type'] == 'Fotos'){
          echo "var id = addImagesInput(".count($value['Fotos']).");
          var i = 1;";
          foreach ($value['Fotos'] as $foto) {
            echo "var blahImg = document.getElementById('blah-' + id + '-' + i );
            blahImg.src = 'uploads/" .$foto. "';
            blahImg.classList.remove('display-none');

            list = new DataTransfer();
            file = new File(['content'], 'uploads/".$foto. "');
            list.items.add(file);
            myFileList = list.files;
            document.getElementById('img-input-' + id + '-' + i ).files = myFileList;
        ...
        }else if($value['Type'] == 'Foto'){
          echo "var id = addSingleImgInput();
          var blahImg = document.getElementById('blah-' + id);
          blahImg.src = 'uploads/" .$value['Foto']. "';
          blahImg.classList.remove('display-none');
          blahImg.parentElement.getElementsByTagName('svg')[0].classList.add('display-none');

          list = new DataTransfer();
          file = new File(['content'], 'uploads/".$value['Foto']. "');
          list.items.add(file);
          myFileList = list.files;
          document.getElementById('img-input-' + id).files = myFileList;
          list = '';
          file = '';
          myFileList = '';";
        }
